...
function tradeUnlock(self, args, cb) {
  ...
  if (!kcCreds) {
    self.prompt({
      ...
      self.log(chalk.green(' KuCoin API credentials ') + chalk.bold.green('unlocked') + chalk.green(' for this Trinity session.'))
      cb()
    })
  } else {
    self.log(chalk.green(' KuCoin API credentials already ') + chalk.bold.green('unlocked.'))
    cb()
  }
}
...
function tradeLock(self, args, cb) {
  let kcCreds = g.get('kucoin')
  if (!kcCreds) {
    self.log(chalk.green(' KuCoin API credentials already ') + chalk.bold.green('locked.'))
    cb()
  } else {
    kcCreds = false
    g.set('kucoin', kcCreds)
    self.log(chalk.green(' KuCoin API credentials ') + chalk.bold.green('locked') + chalk.green(' for this Trinity session.'))
    cb()
  }
}
