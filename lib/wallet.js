      ...
      let transactionHistory = neonjs.getTransactionHistory(network.get(), address)
        ...
        .then(() => {
          ...
          if (claimables) {
            ...
            self.log(' Awaiting claimable information... try again.')
          }
          cb()
        })
    })
  ...
  if (addresses.length <= 0) {
    ...
    self.prompt({
      ...
      if (contacts.length <= 0) {
        self.log(' You need to add a contact to your contact list before you can send.')
        cb()
      } else {
...
function claimAllGas(self, args, cb) {
  ...
  let addressList = []
  if (addresses.length <= 0) {
    self.log(' You need to create or import an address to your wallet before you can claim gas.')
    cb()
  } else {
    for (let i = 0; i < addresses.length; i++) {
      addressList.push((addresses[i].label ? addresses[i].label + ' (' : '') + addresses[i].address + (addresses[i].label ? ')' : ''))
