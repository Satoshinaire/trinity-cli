...
function walletList(self, args, cb) {
  ...
  for (let i = 0; i < addresses.length; i++) {
    if (addresses[i].label) {
      self.log(' * ' + (addresses[i].label ? chalk.bold(addresses[i].label) : '') + ' [ ' + addresses[i].address + (addresses[i].label ? ' ]' : ''))
    } else {
      self.log(' * ' + addresses[i].address)
    }
  }
  cb()
}
...
module.exports.walletCreate = walletCreate
module.exports.walletList = walletList
module.exports.walletRemove = walletRemove
module.exports.walletClear = walletClear
