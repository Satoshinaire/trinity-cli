  ...
  self.prompt([
    ...
    if (inputs.passphrase.length <= 4) {
      ...
      encryptKey(self, newWif, inputs)
        .then((outputs) => {
          winston.log('info', 'Address creation succeeded.', outputs)
          cb()
        })
        .catch((err) => {
          winston.log('error', 'Address creation error.', err)
          cb()
        })
    }
  })
  ...
  self.prompt([
    ...
    if (inputs.existingWif.toString().length != 52) {
      ...
      encryptKey(self, inputs.existingWif.toString(), inputs)
        .then((outputs) => {
          winston.log('info', 'Address import succeeded.', outputs)
          cb()
        })
        .catch((err) => {
          winston.log('error', 'Address creation error.', err)
...
function walletClear(self, args, cb) {
  self.prompt({
    ...
    if (result.doClear) {
      ...
      winston.log('info', 'All addresses cleared from local storage.', defaultConfig.addresses)
    }
    cb()
  })
}
  ...
  if (addresses.length <= 0) {
    ...
    self.prompt({
      ...
      let transactionHistory = neonjs.getTransactionHistory(network.get(), address)
        ...
        .catch((err) => {
          winston.log('error', 'Transaction list for address %s failed to be retrieved.', err)
          cb()
        })
    })
  }
        ...
        self.prompt({
          ...
          self.prompt({
            ...
            self.prompt({
              ...
              winston.log('info', 'Attempting sending transaction.', txnDetails)
              try {
                ...
                winston.log('error', 'Transaction sending failed.', err)
                cb()
              }
            })
          })
    ...
    self.prompt({
      ...
      if (address.indexOf('(') >= 0) {
        ...
        for (let i = 0; i < addresses.length; i++) {
          if (addresses[i].address == txnDetails.fromAddress) {
            txnDetails.encryptedWif = addresses[i].encryptedWif
            txnDetails.canClaim = (addresses[i].claimables.avalable ? true : false)
            addressIndex = i
          }
        }
      }
      ...
      if (txnDetails.canClaim) {
        self.prompt({
          ...
          winston.log('info', 'Attempting claiming transaction.', txnDetails)
          try {
            neonjs.decryptWIF(txnDetails.encryptedWif, result.passphrase)
              .then((decrypted) => {
                self.log(chalk.green(' Passphrase accepted.'))
                ...
                neonjs.doClaimAllGas(network.get(), decrypted)
                  .then((result) => {
                    if (result.result) {
                      ...
                      winston.log('info', 'Claiming transaction succeeded.', result)
                    } else {
                      self.log(chalk.bold.red(' The transaction appears to have failed. Please check your transaction history to confirm.'))
                      winston.log('error', 'Claiming transaction failed.', result)
                    }
                    cb()
                  })
              })
            ...
            self.log(chalk.bold.red(' ' + err.message))
            winston.log('error', 'Claiming transaction failed.', err)
            cb()
          }
        })
      } else {
  ...
  for (let i = 0; i < addresses.length; i++) {
    neonjs.getBalance(network.get(), addresses[i].address)
      .then((result) => {
        ...
        conf.set('addresses', addresses)
        winston.log('silly', 'Updated balance for %s', addresses[i].address, result)
      })
  }
}
