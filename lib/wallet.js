    ...
    self.prompt({
      ...
      let transactionHistory = neonjs.api.getTransactionHistory(network.get(), address)
        ...
        .then(() => {
          ...
          self.log('')
          if (claimables) {
            let gasToClaim = claimables.total_claim / 100000000
            let gasAvailable = (claimables.total_claim - claimables.total_unspent_claim) / 100000000
...
function walletUpdateTokens(self) {
  let deferred = q.defer()
...
function walletUpdateBalances(self) {
  ...
  for (let i = 0; i < addresses.length; i++) {
    neonjs.api.getBalance(network.get(), addresses[i].address)
        ...
        for (let hash in tokens) {
          neonjs.nep5.getTokenBalance(rpc_node, hash, addresses[i].address)
            .then((result) => {
              ...
              conf.set('addresses', addresses)
              winston.log('info', 'Updated token balances for %s', hash, {
                metadata: result
              })
              deferred.resolve(true)
            })
            .catch((err) => {
              winston.log('err', 'Failed token info', {
                metadata: err
              })
      ...
      .catch((err) => {
        winston.log('debug', 'Balance update failed.', {
          metadata: err
        })
        deferred.resolve(false)
      })
...
function walletUpdateClaimables(self) {
  ...
  for (let i = 0; i < addresses.length; i++) {
    neonjs.api.getClaimAmounts(network.get(), addresses[i].address)
      ...
      .catch((err) => {
        winston.log('debug', 'Claimables update failed.', {
          metadata: err
        })
        deferred.resolve(false)
      })
...
module.exports.updateTokens = walletUpdateTokens
