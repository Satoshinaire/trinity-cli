    ...
    for (let i = 0; i < addresses.length; i++) {
      ...
      for (let asset in addresses[i].balances) {
        if (asset != 'unspent') {
          ...
          row[asset] = chalk.green(addresses[i].balances[asset])
          totalBalances[asset] = totalBalances[asset] + addresses[i].balances[asset]
        }
      }
      tableData.push(row)
    }
    self.log(totalBalances)
...
function walletShowBalances(self, args, cb) {
  let addresses = conf.get('addresses')
  let addressList = []
  let addressIndex
  if (addresses.length <= 0) {
    ...
    self.prompt({
      type: 'list',
      name: 'addressSelection',
      message: chalk.green('Which address would you like to see the balance of?'),
      choices: addressList
    }, (result) => {
      let address = result.addressSelection;
      if (address.indexOf('(') >= 0) {
      ...
      for (let asset in addresses[addressIndex].balances) {
        if (asset != 'unspent') {
          let row = {
            Asset: chalk.green(asset),
            Balance: chalk.green(addresses[addressIndex].balances[asset])
          }
          tableData.push(row)
        }
      }
...
module.exports.showBalances = walletShowBalances
