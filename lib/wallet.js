    ...
    neonjs.api.getBalance(network.get(), address)
      .then((addressResult) => {
        ...
        for (let i = 0; i < indexedTokens.length; i++) {
          tokenBalances.push(updateTokenBalance(indexedTokens[i], address))
        }
        return Promise.all(tokenBalances)
          .then((tokenResult) => {
            for (let i = 0; i < result.length; i++) {
              addressResult[indexedTokens[i]] = tokenResult[i]
            }
  ...
  return Promise.all(addressClaimables)
    .then((result) => {
      for (let i = 0; i < result.length; i++) {
        ...
        winston.log('silly', 'Updated claimables for %s', addresses[i].address, {
          metadata: result
        })
      }
      gConf.addresses = addresses
