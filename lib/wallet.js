...
function checkForLedger(self) {
  ...
  let ledgerNode = ledgerComms.list_async()
    .then((devices) => {
      if (devices.length > 0) {
        deferred.resolve(devices)
      } else {
        deferred.reject(devices)
      }
    })
    ...
    .then((devices) => {
      ledgerComms.create_async(5000)
        .then((comm) => {
          ...
          //self.log(message.toString('hex'))
          comm.exchange(message.toString('hex'), validStatus)
            .then((response) => {
              comm.device.close()
              let publicKey = response.substring(0, 130)
              //self.log(publicKey)
              //self.log(neonjs.wallet.isPublicKey(publicKey))
              let newPublicKey = getPublicKeyEncoded(publicKey)
              //self.log(newPublicKey)
              //self.log(neonjs.wallet.isPublicKey(newPublicKey))
              let scriptHash = neonjs.wallet.getScriptHashFromPublicKey(newPublicKey)
              //self.log(scriptHash)
